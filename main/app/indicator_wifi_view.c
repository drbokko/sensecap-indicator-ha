...
static void event_wifi_connect_join(lv_event_t* e) {
	...
	if(code == LV_EVENT_CLICKED)
	{
		...
		// wifi_list_init();
		esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_WIFI_CONNECT, &cfg, sizeof(cfg),
						  portMAX_DELAY);
	}
}
		...
		if(ui_wifi_connecting != NULL && ta != NULL)
		{
			...
			if(password != NULL)
			{
				if((strlen(password) >= 8) && !password_ready)
				{
					...
					lv_obj_set_style_text_color(lv_obj_get_child(join_btn, 0), lv_color_hex(0x529d53),
				...
				if((strlen(password) < 8) && password_ready)
				{
					...
					lv_obj_set_style_text_color(lv_obj_get_child(join_btn, 0), lv_color_hex(0xFFFFFF),
												LV_PART_MAIN | LV_STATE_DEFAULT);
				}
			}
		}
...
static lv_obj_t* create_wifi_connecting(lv_obj_t* parent, const char* p_ssid, bool have_password) {
	...
	if(have_password)
	{
		...
		// lv_obj_set_style_text_font(input_password_title, &ui_font_font0,
		// LV_PART_MAIN| LV_STATE_DEFAULT);
		...
		lv_obj_add_event_cb(ui_password_input, event_wifi_password_input, LV_EVENT_VALUE_CHANGED,
							(void*)wifi_connect_join);
...
/**
 * @brief message that tells you connected connected or not
 *
 * @param p_msg
 */
...
static void event_wifi_delete(lv_event_t* e) {
	...
	if(code == LV_EVENT_CLICKED)
	{
		...
		esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_WIFI_LIST_REQ, NULL, 0,
						  portMAX_DELAY); // updata wifi list
	}
}
...
static void __view_event_handler(void* handler_args, esp_event_base_t base, int32_t id, void* event_data) {
	switch(id)
	{
		...
		{
			ESP_LOGI(TAG, "VIEW_EVENT_SCREEN_START");
		...
		case VIEW_EVENT_WIFI_LIST_START: //  retrive wifi nearby
			ESP_LOGI(TAG, "VIEW_EVENT_WIFI_LIST_START");
			if(ui_wifi_list) // the older infomation exists
			{
				lv_port_sem_take();
				lv_obj_add_flag(ui_wifi_list,
								LV_OBJ_FLAG_HIDDEN); // then hidden it first
				lv_obj_clear_flag(ui_wifi_scan_wait, LV_OBJ_FLAG_HIDDEN);
				/*when retrived, VIEW_EVENT_WIFI_LIST is receieved*/
				lv_port_sem_give();
			}
			break;
		...
		case VIEW_EVENT_WIFI_LIST: /* get the wifi list from wifi model */
		{
			ESP_LOGI(TAG, "event: VIEW_DATA_WIFI_LIST");
			// Wi-fi screen didn't created
			lv_port_sem_take();
			...
			{
				ESP_LOGI(TAG, "ssid:%s, rssi:%d, auth mode:%d", p_list->aps[i].ssid, p_list->aps[i].rssi,
						 p_list->aps[i].auth_mode);
				if(p_list->is_connect)
				{
					if(strcmp(p_list->aps[i].ssid, p_list->connect.ssid) != 0)
					{
						create_wifi_item(ui_wifi_list, p_list->aps[i].ssid, p_list->aps[i].auth_mode,
										 p_list->aps[i].rssi, false);
					}
				}
				else
				{
					create_wifi_item(ui_wifi_list, p_list->aps[i].ssid, p_list->aps[i].auth_mode, p_list->aps[i].rssi,
									 false);
				}
			}
			lv_obj_clear_flag(ui_wifi_list, LV_OBJ_FLAG_HIDDEN); // display the wifi list panel
			lv_obj_add_flag(ui_wifi_scan_wait, LV_OBJ_FLAG_HIDDEN); // hide the spinner
			lv_port_sem_give();
			break;
		}
